...
data_dir = '/scratch/jkbrook/Deep_Learning/HW_2/MLDS_hw2_1_data/'

def data_preprocess(data_dir):
    
    with open(data_dir + 'training_label.json', 'r') as f:
        file = json.load(f)
    ...
    other_tokens = [('<PAD>', 0), ('<SOS>', 1), ('<EOS>', 2), ('<UNK>', 3)]
    i2w = {i + len(other_tokens): w for i, w in enumerate(word_dict)}
    w2i = {w: i + len(other_tokens) for i, w in enumerate(word_dict)}
    
    for token, index in other_tokens:
        i2w[index] = token
        w2i[token] = index
...
def Annotate(label_file, word_dict, w2i, data_dir):
    
    label_json = data_dir + label_file
...
def avi(files_dir, data_dir):
    
    avi_data = {}
    training_feats = data_dir + files_dir
...
class train_data(Dataset):
    ...
    def __init__(self, label_file, files_dir, word_dict, w2i):
        ...
        self.avi = avi(label_file, data_dir)
        self.w2i = w2i
        self.data_pair = Annotate(files_dir, word_dict, w2i, data_dir)
...
def main():
    ...
    i2w, w2i, word_dict = data_preprocess(data_dir)
    
    with open('i2w.pickle', 'wb') as handle:
        pickle.dump(i2w, handle, protocol = pickle.HIGHEST_PROTOCOL)
    
    label_file = '/training_data/feat'
    files_loc = 'training_label.json'
    train_dataset = train_data(label_file, files_loc, word_dict, w2i)
    train_dataloader = DataLoader(dataset = train_dataset, batch_size=128, shuffle=True, num_workers=4, collate_fn=minibatch)
    
    epochs_n = 200

    encoder = Encoder()
    decoder = Decoder(512, len(i2w) +4, len(i2w) +4, 1024, 0.3)
    model = Seq2SeqModel(encoder=encoder, decoder=decoder)
    ...
    torch.save(model, "{}.h5".format('trained_model'))
